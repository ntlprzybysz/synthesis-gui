...
def check_task_status(session_key: str) -> int:
    ...
    def _get_current_task_status(log, entry: str) -> int:
        """
        Returns the current task status as an integer code based on the given entry from the log.
        The time-consuming stages give in-between updates based on their typical processing times.
        The in-between updates are only visible on front-end and aren't added to log.
        """
        if "tasks" in entry and "Created new project. Sending for processing" in entry:
            ...
            time = _get_time_in_state(log, 9)  # first time the state is visited is at 9
            ...
            time = _get_time_in_state(log, 41)  # first time the state is visited is at 41
            if time >= 0 and time < 5:
                return 41
            elif time >= 5 and time < 15:
                return 53
            elif time >= 15 and time < 40:
                return 61
            elif time >= 40 and time < 75:
                return 75
    ...
    while (time_in_state < 300) and (state_changed == False):
        ...
        current_state = _get_current_task_status(log, last_entry)
        if current_state < 0:
            logger.error(
