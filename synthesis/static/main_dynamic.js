...
/**
 * Changes symbols and speakers available in the drop down menus based on the selected model.
 */
document.addEventListener("DOMContentLoaded", function () {
    var modelDropdown = document.getElementById("model-select-field");
    var voiceDropdown = document.getElementById("voice-select-field");
    var ipaSymbolRow = document.getElementById("ipa-symbol-row");
    ...
    modelDropdown.addEventListener("change", function () {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                // Clear existing options in the voice dropdown
                while (voiceDropdown.firstChild) {
                    voiceDropdown.removeChild(voiceDropdown.firstChild);
                }

                // Populate the voice dropdown with new options
                ...
                // Clear existing symbols in the IPA table
                while (ipaSymbolRow.firstChild) {
                    ipaSymbolRow.removeChild(ipaSymbolRow.firstChild);
                }

                // Populate the symbols with new options
                data.symbols.forEach(function (symbol) {
                    ...
                    ipaSymbolRow.appendChild(button);
                });
            }
        };
        xhr.open('GET', '/get_model_data/?model=' + selectedModel, true);
